...
int nAnonymizeVirtualAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Virtual
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Virtual
    // Mac: ~/Library/Application Support/Virtual
    // Unix: ~/.virtual
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Virtual";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "virtual.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "virtuald.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong VIRTUAL will not work properly.");
